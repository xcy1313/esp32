...
void vesync_notify_app_net_result(char *trace_id,int err_code ,char *err_describe,int server_err_code)
{
    ...
    buflen = sprintf((char *)upload_buf, "{\"uri\":\"/beginConfigReply\",\"err\":%d,\"description\":\"%s\",\"server_code\":%d,\"traceId\":\"%s\",\"routerMac\":\"%s\",\"deviceRSSI\":\"%d\",\"firmVersion\":\"%s\"}",
            err_code,err_describe,server_err_code,trace_id,ap_mac_addr,12,FIRM_VERSION);
...
static void vesync_Handle_ConfigNetJson(net_info_t *info,cJSON *json)
{
	...
	if(true == cJSON_IsString(pid)){
        ...
        if(0 == strcmp((char *)info->mqtt_config.pid,DEV_PID)){
            mask |=0x1;
        }else{
	...
	if(true == cJSON_IsString(configKey)){
        ...
        key_size = strlen((char *)info->mqtt_config.configKey);
    ...
    if(true == cJSON_IsString(serverUrl)){
        ...
        if(strlen((char*)info->station_config.server_url) != 0){
            mask |=0x10;
	...
	if(true == cJSON_IsString(wifiSSID)){
        ...
        if(strlen((char *)info->station_config.wifiSSID) !=0){
            mask |=0x40;
        }else{
    ...
    if((mask & 0x7ff) == 0x7ff){    //设备已拿到并且校验正确的配网信息,开始配网
        ...
        vesync_set_device_status(DEV_CONFNET_OFFLINE);      //状态调整为配网失败
...
static void vesync_blufi_recv_custom_message(net_info_t *info,const char *data, int length)
{
	...
	if(true == cJSON_IsString(uri)){
		...
		if(!strcmp(uri->valuestring, "/beginConfigRequest")){		    //开始配网
            ...
            if(vesync_get_device_status() == DEV_CONFNET_ONLINE){
                vesync_reply_response("/queryDeviceNet",ERR_CONFIG_HTTPS_NET_SUCCESS, "ERR_CONFIG_HTTPS_NET_SUCCESS");
            }else{
                vesync_reply_response("/queryDeviceNet",ERR_CONNECT_HTTPS_SERVER_FAIL, "ERR_CONNECT_HTTPS_SERVER_FAIL");
            }
        }else{
                                    ...
                                    vesync_bt_status_cb_t bt_status_cb, bt_recv_cb_t cb)
{
    ...
    strncpy((char *)&sw_version[6],project_version,4);
