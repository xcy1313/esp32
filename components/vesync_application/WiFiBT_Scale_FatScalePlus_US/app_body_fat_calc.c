...
#define FAT_LOW_16_WOMAN(F,A,B)         ((173870/(F)+442-55642100/((B)*(F))))
        ...
        case 0: //男人
            if(mask & 0x0001){    //仅仅处理部分
                if(config->user_mode == NORMAL_MODE){   //普通人模式
                    if(config->age >= 16){
                        calc_fat = 120090.0/((double)(config->imped_value))+0.676*((double)(config->age))+376.0-50679100.0/(((double)calc_bmi)*((double)(config->imped_value)));
                    }else{
                        calc_fat = 120090/((double)config->imped_value)+566-(11.21*((double)(config->age)))-50679100/(((double)calc_bmi)*((double)(config->imped_value)));
                    }
                }else{
                    if(config->age >= 16){
            ...
            if(mask & 0x0001){
                if(config->user_mode == NORMAL_MODE){   //普通人模式
                    if(config->age >= 16){
                        ...
                        //calc_fat = FAT_LOW_16_WOMAN(p_weitht->imped_value,config->age,calc_bmi);
                        calc_fat = 173870/((double)config->imped_value)+442-55642100/(((double)calc_bmi) *((double)config->imped_value));
                    }
                }else{
                    if(config->age >= 16){
                ...
                if(len == 0){                           //第一次使用
                    ...
                    for(;i<(len/sizeof(user_config_data_t));i++){
                        if((abs(user_list[i].weight_kg - new_kg) <= MAX_WEIGHT)){              //前后两次体重小于3kg
                            ...
                            //ESP_LOGI(TAG, "store flash user config account:[0x%04x],kg:[%d],imped:[%d],number:[%d]",user_list[i].account,user_list[i].weight_kg,user_list[i].imped_value,i);
                            user_cnt++;
                        }
                    ...
                    if(user_cnt !=0){
                        ...
                        list_number = find_near_data(&backup_user_list,user_cnt,new_kg);
                        ...
                        if(!vesync_flash_write(USER_HISTORY_DATA_NAMESPACE,backup_user_list[list_number].user_store_key,(user_history_t *)&history ,sizeof(user_history_t))){
...
#else
                    p_weitht->weight = 6665;
                    p_weitht->imped_value = 563;
                    user_fat_data_t  resp_fat_data ={0};
                    user_history_t history = {0};
